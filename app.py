...
def get_total_races(season):
    ...
    try:
        ...
        print(f"Error fetching total races: {e}")
        return None

def fetch_race_data(season, race_number):
    url = f'https://ergast.com/api/f1/{season}/{race_number}/results.json'
    try:
        response = requests.get(url, timeout=10)
        response.raise_for_status()
...
def predict():
    ...
    if total_races is None:
        return render_template('result.html', error="Could not connect to the Ergast API. Please check your internet connection or try again later.")
